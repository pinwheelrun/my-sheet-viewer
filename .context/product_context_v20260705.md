...
### Current state (Vue port phase 1 in progress)

- `App.vue` currently acts as the single canvas viewer, scaled to `devicePixelRatio`
- Top bar: "Open PDF" button, dynamic page strip, total page count
- Page strip window calculated dynamically from actual `offsetWidth` of the
  strip element; `nextTick` awaited before reading `offsetWidth`
- Tap zones and strip visible only after a file is loaded
- `pointerdown` + `e.preventDefault()` on tap zones for immediate response
- `touch-action: none` on tap zone elements
- Per-zone debounce cooldown ~400ms
...
**Phase 3 — Landscape mode (normal mode only)**
...
**Phase 7 — Sequence mode in Viewer + Landscape**
